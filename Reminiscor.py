...
class LoginWindow(Screen):
    ...
    def welcome_screen(self, *_):
        if not CheckUser():
            design = Welcome()
            welcome_pop = Popup(title='', content=design, size_hint=(None, None), size=(
                400, 500), separator_height=0, background='UI/Welcome.png')
            welcome_pop.open()
    ...
    def user_error_popup(self):
        design = UserError()
        UserExists = Popup(title='User Error Encountered!', title_align='center', content=design, size_hint=(None, None), size=(
            400, 200), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x200.png', auto_dismiss=False)
        UserExists.open()
        design.ids.close.bind(on_release=UserExists.dismiss)

    def signup_pop(self, *_):
        design = SignUp_Pop()
        self.win = Popup(title='Sign-Up Screen', title_align='center', content=design, size_hint=(None, None),
                         size=(400, 450), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x450.png')
        self.win.open()
    ...
    def Login_Authenticate(self):
        ...
        Master_Password = ''
        if CheckUser():
            # To check if main credential file exists
            if CheckCredentials(self.username.text, self.p.text, Master_Password_key):
                ...
                if (os.path.isdir(ReminiscorFiles_Dir(''))):
                    File_dir0 = os.path.expanduser('~') + '/Reminiscor Files'
                    direc0 = "Export"
    ...
    def help1(self):
        design = Help1()
        help1win = Popup(title='Help', content=design, size_hint=(None, None), size=(400, 250), separator_color=[
    ...
    def check(self):
        if CheckMainPassword(self.user.text, self.p1.text, self.p2.text):
            ...
            try:
                ...
                ic(os.path.join(File_dir0, direc0))
                os.makedirs(os.path.join(File_dir0, direc0))
                os.makedirs(os.path.join(File_dir1, direc1))
                os.makedirs(os.path.join(File_dir2, direc2))
            ...
            error_win = Popup(title='Sign-Up Failed', content=errorpop, size_hint=(None, None), size=(
                400, 250), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x250.png')
            error_win.open()
...
class MainWindow(Screen):
    ...
    def clear(self):
        ...
        Master_Password_key = pbkdf2.PBKDF2(
            str(randint(0, 10000000)), salt).read(32)
    ...
    def p_size_popup(self):
        ...
        win = Popup(title='Entry Error!', content=design, size_hint=(None, None), size=(400, 400), separator_color=[
                    0, 171/255, 174/255, 1], background='UI/popup400x400.png', auto_dismiss=False)
        win.open()
        design.ids.close.bind(on_release=win.dismiss)
    ...
    def authenticate(self):
        if(CheckUser()):
            ...
            if(CheckCredentials(self.ids.username.text, self.ids.p.text, Master_Password_key)):
                ...
                export_extension = Popup(title='Choose Entries to Share', title_align='center', content=export_design, size_hint=(
                    None, None), size=(400, 575), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x400.png', auto_dismiss=False)
                export_extension.open()
                export_design.ids.close.bind(
                    on_release=export_extension.dismiss)
            else:
    ...
    def import_shared(self, username, Master_Password_key, *args):
        try:
            import_result = Import(
                self.ids.commonpassw.text, username, Master_Password_key)
        except:
            result = resultpop()
...
class Login_Popup_import(FloatLayout):
    def authenticate(self):
        if(CheckUser()):
            ...
            if(CheckCredentials(self.ids.username.text, self.ids.p.text, Master_Password_key)):
                ...
                win.open()
                design.ids.close.bind(on_release=win.dismiss)
                design.ids.submit.bind(on_release=partial(
                    self.import_process, design, win))
            else:
                self.ids.label.text = 'Authentication Failed!'
    ...
    def import_process(self, design, win, *args):
        global Master_Password_key
        win.dismiss()
        Clock.schedule_once(partial(design.import_shared,
                                    self.ids.username.text, Master_Password_key), 0.15)
...
class Password_Screen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        Backbtn = Button(text='Go Back', size_hint=(None, None), size=(
            150, 96), pos_hint={'x': 0, 'y': 0}, on_release=self.screenswitch)
    ...
    def showlist(self, *largs):
        ...
        title_label = Label(text='[u][b]Entry List:[/b][/u]', markup=True,
                            size_hint_y=None, height=60, font_size=20, color=[0, 171/255, 174/255, 1])
        parent.add_widget(title_label)
        if os.path.isfile(HomeDir('Data3.dat')):
            global Master_Password_key
            password_list = ReadDecrypt(
                HomeDir('Data3.dat'), Master_Password_key)
            pass_len = len(password_list)
            if pass_len > 0:
                for i in password_list:
                    entrydata = i.split('qwertyuiop***asdfghjklzxcvbnm')
                    btn = Button(text=entrydata[0], size_hint_y=None, height=60, on_release=partial(
                        self.poppassword, entrydata))
                    btn.background_normal = 'UI/firstlistbutton.png'
                    btn.background_down = 'UI/firstlistbuttondown.png'
                    layout1.add_widget(btn)
                root = ScrollView(size_hint=(1, 0.9), size=(Window.width, Window.height), bar_margin=2, scroll_type=[
                                  'bars', 'content'], bar_width=8, bar_color=[0, 171/255, 174/255, 1])
                root.add_widget(layout1)
                parent.add_widget(root)
                self.mainlayout.add_widget(parent)
            else:
                btn = Button(text='No passwords yet!', size_hint_y=None,
                             height=60, color=[204/255, 0, 0, 1])
    ...
    def searchresult(self, instance):
        ...
        if result == []:
            ...
            search = Popup(title='Search result', title_align='center', content=design, size_hint=(None, None), size=(
                400, 400), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x400.png', auto_dismiss=False)
            search.open()
            design.ids.close.bind(on_release=partial(
                self.DismissAndTriggerCancel, design, search))
        else:
            design = search_popup()
            ...
            search = Popup(title='Search result', title_align='center', content=design, size_hint=(None, None), size=(
                400, 400), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x400.png')
            search.open()
            design.ids.close.bind(on_release=partial(
                self.DismissAndTriggerCancel, design, search))
            design.ids.delete.bind(on_release=partial(
                self.DeleteAndRefresh, design, search))
            design.ids.edit.bind(on_release=partial(
                self.editflow, design, result))
        self.searchbar.text = ''

    def Backup(self):
        design = Backup()
        Backupwin = Popup(title='Backup Your Data', title_align='center', content=design, size_hint=(
            None, None), size=(400, 400), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x400.png')
        Backupwin.open()
    ...
    def poppassword(self, entrydata, *args):
        ...
        entry = Popup(title='Entry Information', title_align='center', content=design, size_hint=(None, None), size=(
            400, 400), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x400.png', auto_dismiss=False)
        entry.open()
        design.ids.close.bind(on_release=partial(
            self.DismissAndTriggerCancel, design, entry))
        design.ids.delete.bind(on_release=partial(
            self.DeleteAndRefresh, design, entry))
        design.ids.edit.bind(on_release=partial(
            self.editflow, design, entrydata))

    def editflow(self, design, entrydata, instance):
        editconfirm = Edit_Confirmation()
        editwin = Popup(title='Edit Confirmation', content=editconfirm, size_hint=(None, None), size=(
            400, 200), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x200.png', auto_dismiss=False)
        editwin.open()
        editconfirm.ids.close.bind(on_release=editwin.dismiss)
        editconfirm.ids.yes.bind(on_release=partial(
            self.confirm2, design, entrydata, editwin))
    ...
    def DeleteAndRefresh(self, design, entry, instance):
        ...
        deletewin = Popup(title='Delete Confirmation', content=deletepop, size_hint=(None, None), size=(
            400, 200), separator_color=[0, 171/255, 174/255, 1], background='UI/popup400x200.png', auto_dismiss=False)
        deletewin.open()
        deletepop.ids.close.bind(on_release=deletewin.dismiss)
        deletepop.ids.yes.bind(on_release=partial(
            self.confirm1, design, entry, deletewin))

...
#   REFLECTOR IS ALSO DIFFERENT EACH LETTER IS MUTUALLY MATCHED
    ...
    rotor1 = list(string.punctuation+string.ascii_letters +
                  string.digits+' '+'\x1f')
    rotor2 = list(string.punctuation+string.ascii_letters +
                  string.digits+' '+'\x1f')
    rotor3 = list(string.punctuation+string.ascii_letters +
                  string.digits+' '+'\x1f')
    shuffle(rotor1)
    shuffle(rotor2)
    shuffle(rotor3)
    sep = list('qwertyuiop***asdfghjklzxcvbnm')
    setting = plugboard[0]+sep+plugboard[1]+sep+rotor1 + \
        sep+rotor2+sep+rotor3+sep+reflector[0]+sep+reflector[1]
    return ''.join(setting)
...
def KeyDerivationFunction(master_password, skey):
    ...
    sep = '******'
    derivedkey = ''
    derivedkey += key[0]+sep+key[1]+sep+key[2] + \
        sep+key[3]+sep+key[4]+sep+key[5]+sep+key[6]
    # print(derivedkey)
...
def EnigmaMachine(msg_str, enigma_setting_str):
    # stores the five rotors as strings in a list
    enigma = enigma_setting_str.split('qwertyuiop***asdfghjklzxcvbnm')
    plugboard = []
    reflector = []
    ...
    # A list with correct arrangement of characters
    Ordered_List = list(string.punctuation +
                        string.ascii_letters+string.digits+' '+'\x1f')
